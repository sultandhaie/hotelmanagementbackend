...
import { ResrvationStatus, STATUS_CONFIG } from "@/app/type/Reservation";
import { X } from "lucide-react";

interface DataTableProps<TData, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
  onBulkStatusChange?: (rowIndices: number[], newStatus: ResrvationStatus) => void;
}
...
export function DataTable<TData, TValue>({
  ...
  const handleBulkStatusChange = (newStatus: ResrvationStatus) => {
      ...
      {selectedCount > 0 && (
        <div className="flex items-center gap-3 rounded-md border bg-muted/50 px-4 py-2.5 text-sm animate-in fade-in slide-in-from-top-1 duration-150">
          ...
          <div className="flex items-center gap-1.5">
            {(Object.keys(STATUS_CONFIG) as ResrvationStatus[]).map((status) => {
                  ...
                  {row.getVisibleCells().map((cell) => (
                    <TableCell key={cell.id}>
                      {cell.column.id === "status"
                        ? (() => {
                            const status = cell.getValue() as ResrvationStatus;
                            const { label ,color, bg, dot } =
